...
class billingClass:
    def __init__(self,root):
        ...
        p_title = Label(ProductFrame1,text='All Products',
                        font=('times new roman',20,'bold'),bg='#262626',fg='white').pack(side=TOP,fill=X)
        ...
        lbl_search = Label(ProductFrame2,text='Product Name',
                         font=('times new roman',15,'bold'),bg='white').place(x=5,y=45)
        
        txt_search = Entry(ProductFrame2,textvariable=self.var_search,font=('times new roman',15),
                           bg='lightyellow').place(x=129,y=47,width=150,height=22)
        
        btn_search = Button(ProductFrame2,text='Search',font=('times new roman',15),command=self.search,
                            bg='#2196f3',fg='white',cursor='hand2').place(x=285,y=45,width=100,height=25)
        
        btn_show_all = Button(ProductFrame2,text='Show All',font=('times new roman',15),command=self.show,
        ...
        self.productTable = ttk.Treeview(ProductFrame3,columns=("pid","name","price","quantity","status"),
        ...
        self.productTable.column("quantity",width=55)
        ...
        c_title = Label(CustomerFrame,text='Customer Details',
                        font=('times new roman',15,'bold'),bg='lightgray',fg='black').pack(side=TOP,fill=X)
        
        lbl_name = Label(CustomerFrame,text='Name',
        ...
        txt_name = Entry(CustomerFrame,textvariable=self.var_cname,font=('times new roman',13),
                           bg='lightyellow').place(x=70,y=35,width=180)
        
        lbl_contact = Label(CustomerFrame,text='Contact No.',
        ...
        # Calculator Frame
        self.var_cal_inp = StringVar()
        Cal_Frame = Frame(Cal_Cart_Frame,bd=9,relief=RIDGE,bg='white')
        Cal_Frame.place(x=5,y=10,width=268,height=340)

        txt_cal_input = Entry(Cal_Frame,textvariable=self.var_cal_inp,font=('arial',15,'bold'),width=21,bd=10,relief=GROOVE,
                              state='readonly',justify=RIGHT)
        txt_cal_input.grid(row=0,columnspan=4)

        btn_7 = Button(Cal_Frame,text='7',font=('arial',15,'bold'),command=lambda:self.get_input(7),bd=5,width=4,pady=10,
        cursor='hand2').grid(row=1,column=0)
        btn_8 = Button(Cal_Frame,text='8',font=('arial',15,'bold'),command=lambda:self.get_input(8),bd=5,width=4,pady=10,
        cursor='hand2').grid(row=1,column=1)
        btn_9 = Button(Cal_Frame,text='9',font=('arial',15,'bold'),command=lambda:self.get_input(9),bd=5,width=4,pady=10,
        cursor='hand2').grid(row=1,column=2)
        btn_sum = Button(Cal_Frame,text='+',font=('arial',15,'bold'),command=lambda:self.get_input('+'),bd=5,width=3,pady=10,
        cursor='hand2').grid(row=1,column=3)
        
        btn_4 = Button(Cal_Frame,text='4',font=('arial',15,'bold'),command=lambda:self.get_input(4),bd=5,width=4,pady=10,
        cursor='hand2').grid(row=2,column=0)
        btn_5 = Button(Cal_Frame,text='5',font=('arial',15,'bold'),command=lambda:self.get_input(5),bd=5,width=4,pady=10,
        cursor='hand2').grid(row=2,column=1)
        btn_6 = Button(Cal_Frame,text='6',font=('arial',15,'bold'),command=lambda:self.get_input(6),bd=5,width=4,pady=10,
        cursor='hand2').grid(row=2,column=2)
        btn_sub = Button(Cal_Frame,text='-',font=('arial',15,'bold'),command=lambda:self.get_input('-'),bd=5,width=3,pady=10,
        cursor='hand2').grid(row=2,column=3)
        
        btn_1 = Button(Cal_Frame,text='1',font=('arial',15,'bold'),command=lambda:self.get_input(1),bd=5,width=4,pady=10,
        cursor='hand2').grid(row=3,column=0)
        btn_2 = Button(Cal_Frame,text='2',font=('arial',15,'bold'),command=lambda:self.get_input(2),bd=5,width=4,pady=10,
        cursor='hand2').grid(row=3,column=1)
        btn_3 = Button(Cal_Frame,text='3',font=('arial',15,'bold'),command=lambda:self.get_input(3),bd=5,width=4,pady=10,
        cursor='hand2').grid(row=3,column=2)
        btn_mul = Button(Cal_Frame,text='*',font=('arial',15,'bold'),command=lambda:self.get_input('*'),bd=5,width=3,pady=10,
        cursor='hand2').grid(row=3,column=3)

        btn_0 = Button(Cal_Frame,text='0',font=('arial',15,'bold'),command=lambda:self.get_input(0),bd=5,width=4,pady=15,
        ...
        btn_equ = Button(Cal_Frame,text='=',font=('arial',15,'bold'),command=self.perform_cal,bd=5,width=4,pady=15,
        cursor='hand2').grid(row=4,column=2)
        btn_div = Button(Cal_Frame,text='/',font=('arial',15,'bold'),command=lambda:self.get_input('/'),bd=5,width=3,pady=15,
        ...
        self.CartTable = ttk.Treeview(cart_Frame,columns=("pid","name","price","quantity"),yscrollcommand=scroll_y.set,
        ...
        self.CartTable.bind('<ButtonRelease-1>',self.getData_cart)
        ...
        lbl_p_name = Label(Add_Cart_Widgets_Frame, text='Product Name',
                       font=('times new roman',15),bg='white').place(x=5,y=5)
        
        txt_p_name = Entry(Add_Cart_Widgets_Frame, textvariable=self.var_pname,
                       font=('times new roman',15),
                       bg='lightyellow',state='readonly').place(x=5,y=35,width=190,height=22)
        
        lbl_p_price = Label(Add_Cart_Widgets_Frame, text='Price Per Qty',
                       font=('times new roman',15),bg='white').place(x=230,y=5)
        
        txt_p_price = Entry(Add_Cart_Widgets_Frame, textvariable=self.var_price,
                       font=('times new roman',15),
                       bg='lightyellow',state='readonly').place(x=230,y=35,width=150,height=22)
        
        lbl_p_qty = Label(Add_Cart_Widgets_Frame, text='Quantity',
                       font=('times new roman',15),bg='white').place(x=390,y=5)
        
        txt_p_qty = Entry(Add_Cart_Widgets_Frame, textvariable=self.var_qty,
                       font=('times new roman',15),
                       bg='lightyellow').place(x=390,y=35,width=120,height=22)
        
        self.lbl_inStock = Label(Add_Cart_Widgets_Frame, text='In Stock',
                       font=('times new roman',15),bg='white')
        self.lbl_inStock.place(x=5,y=70)

        btn_clrear_cart = Button(Add_Cart_Widgets_Frame,text='Clear',font=('times new roman',15,'bold'),
                            bg='lightgray',cursor='hand2',command=self.clear_cart).place(x=180,y=70,width=150,height=30)
        
        btn_add_cart = Button(Add_Cart_Widgets_Frame,text='Add | Update Cart',font=('times new roman',15,'bold'),
                              bg='orange',cursor='hand2',command=self.add_update_cart).place(x=340,y=70,width=180,height=30)
        ...
        B_title = Label(billFrame,text='Customer Bill Area',
                        font=('times new roman',20,'bold'),bg='orange',fg='white').pack(side=TOP,fill=X)
        ...
        self.lbl_discount = Label(billMenuFrame,text='Discount\n[10%]',font=('times new roman',15,'bold'),bg='#8bc34a',fg='white')
        self.lbl_discount.place(x=124,y=5,width=120,height=70)

        self.lbl_net_pay = Label(billMenuFrame,text='Net Pay\n[0]',font=('times new roman',15,'bold'),bg='#607d8b',fg='white')
        self.lbl_net_pay.place(x=246,y=5,width=160,height=70)

        btn_print = Button(billMenuFrame,text='Print',command=self.print_bill,cursor='hand2',font=('times new roman',15,'bold'),
                           bg='red',fg='white')
        btn_print.place(x=2,y=80,width=120,height=50)

        btn_clear_all = Button(billMenuFrame,text='Clear All',cursor='hand2',command=self.clear_all,font=('times new roman',15,
                            'bold'),bg='gray',fg='white')
        btn_clear_all.place(x=124,y=80,width=120,height=50)

        btn_generate = Button(billMenuFrame,text='Generate/\nSave Bill',command=self.generate_bill,cursor='hand2',
                              font=('times new roman',15,'bold'),bg='#009688',fg='white')
        btn_generate.place(x=246,y=80,width=160,height=50)

        # Footer--------------------------------------------------------
        footer = Label(self.root,text="IMS-Inventory Management System | Developed By Vivek Kulkarni\nFor Any Technical Issues Contact: 7975251112",
                      font=('times new roman',10,'bold'),bg="#4d636d", 
...
# ---------------------------------------All Function---------------------------------------------------
    def get_input(self,num):
        xnum = self.var_cal_inp.get()+str(num)
        self.var_cal_inp.set(xnum)

    def clear_cal(self):
        self.var_cal_inp.set("")

    def perform_cal(self):
        result = self.var_cal_inp.get()
        self.var_cal_inp.set(eval(result))
    ...
    def add_update_cart(self):
        if self.var_pid.get() == '':
            ...
            messagebox.showerror('Error','Entered Invalid Quantity',parent=self.root)
    ...
    def bill_updates(self):
        ...
        self.lbl_net_pay.config(text=f'Ney Pay\n{str(self.net_pay)}')
    ...
    def bill_top(self):
        ...
        bill_top_temp = f'''
\t      Vivek Inventory Shop
      Phone No.7975251112 , Vijayapura-586103
{str("="*46)}
